                ...
                if (board[i][j] == 0 || board[i][j] == -1) {
                    for (int dig = 1; dig <= n; dig++) {
                        if (isSafe(i, j, dig, board)) {
                            board[i][j] = dig;
                            setUpdated(i, j); // Mark cell as updated
                            updateBoard(board, i, j); // Pass updated cell coordinates
                            try {
                                Thread.sleep(getDelay()); // Wait for the delay from the slider
                            ...
                            if (solveSudoku(board)) return true;
                                ...
                                board[i][j] = -1;
                                resetUpdated(i, j); // Reset updated cell
                                updateBoard(board, i, j); // Pass updated cell coordinates
                                try {
                                    Thread.sleep(getDelay()); // Wait for the delay from the slider
                                } catch (InterruptedException e) {
